...
-- Inserir provedores padrão para organizações existentes (desabilitados por padrão)
-- Descomente se quiser criar provedores padrão para todas as organizações

/*
INSERT INTO payment_providers (org_id, name, display_name, is_active, config)
SELECT 
  id as org_id,
  'stripe' as name,
  'Stripe' as display_name,
  false as is_active,
  '{"api_key": "", "webhook_secret": ""}' as config
FROM organizations
...
COMMENT ON TABLE payment_providers IS 'Configuração dos provedores de pagamento por organização';
COMMENT ON TABLE payment_transactions IS 'Transações de pagamento processadas';
COMMENT ON TABLE payment_subscriptions IS 'Assinaturas e pagamentos recorrentes';
COMMENT ON TABLE payment_webhooks IS 'Webhooks recebidos dos provedores de pagamento';
COMMENT ON TABLE payment_audit_logs IS 'Log de auditoria de todas as ações relacionadas a pagamentos';

COMMENT ON COLUMN payment_providers.config IS 'Configurações do provedor (API keys, etc) - deve ser criptografado na aplicação';
COMMENT ON COLUMN payment_providers.priority IS 'Ordem de prioridade para failover (1 = maior prioridade)';
COMMENT ON COLUMN payment_transactions.status IS 'Status: pending, processing, succeeded, failed, canceled, refunded';
COMMENT ON COLUMN payment_subscriptions.status IS 'Status: active, canceled, past_due, unpaid';