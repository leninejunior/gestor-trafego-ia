...
FOR ALL USING (
  EXISTS (
    ...
    WHERE om.organization_id = campaign_squad.campaign_schedules.organization_id
...
FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM organization_members om
    WHERE om.organization_id = campaign_squad.campaign_runs.organization_id
      AND om.user_id = auth.uid()
  )
);
...
FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM campaign_squad.campaign_runs r
    JOIN organization_members om ON om.organization_id = r.organization_id
    WHERE r.id = campaign_squad.approval_requests.run_id
      AND om.user_id = auth.uid()
  )
);
...
FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM organization_members om
    WHERE om.organization_id = campaign_squad.squad_llm_configs.organization_id
      AND om.user_id = auth.uid()
...
FOR ALL USING (
  EXISTS (
    SELECT 1
    FROM organization_members om
    WHERE om.organization_id = campaign_squad.squad_llm_credentials.organization_id
      AND om.user_id = auth.uid()
      AND om.role IN ('owner', 'admin')
  )
);
