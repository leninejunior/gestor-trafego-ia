-- Script FINAL para criar super admin
-- Execute este SQL no Supabase SQL Editor

-- 1. Ver estrutura atual das tabelas
...
-- 4. Criar role super_admin se não existir
...
-- 5. Criar organização se não existir
INSERT INTO organizations (name)
SELECT 'Organização Admin'
WHERE NOT EXISTS (SELECT 1 FROM organizations);

-- 6. Sincronizar user_profiles
...
-- 7. Tornar o PRIMEIRO usuário super admin (usando org_id)
INSERT INTO memberships (user_id, org_id, role, role_id, status, accepted_at)
SELECT 
...
-- 8. Atualizar membership existente se necessário
UPDATE memberships 
SET 
    role = 'super_admin',
...
AND org_id = (SELECT id FROM organizations ORDER BY created_at ASC LIMIT 1);
...
-- 11. Mostrar todos os usuários e suas roles
SELECT 
...
-- 12. Inserir atividade de exemplo
