-- ============================================
-- LIMPEZA COMPLETA - Remove tudo relacionado a alertas de saldo
-- ============================================

-- Remover policies existentes
DROP POLICY IF EXISTS "Users can view balance alerts for their org clients" ON balance_alerts;
DROP POLICY IF EXISTS "Users can manage balance alerts for their org clients" ON balance_alerts;
DROP POLICY IF EXISTS "Users can view whatsapp config for their org" ON whatsapp_config;
DROP POLICY IF EXISTS "Only admins can manage whatsapp config" ON whatsapp_config;
DROP POLICY IF EXISTS "Users can view alert history for their org" ON alert_history;
DROP POLICY IF EXISTS "Users can view recipients for their org clients" ON alert_recipients;
DROP POLICY IF EXISTS "Admins can manage recipients for their org clients" ON alert_recipients;

-- Remover views
DROP VIEW IF EXISTS active_balance_alerts;
DROP VIEW IF EXISTS alert_statistics;

-- Remover triggers
DROP TRIGGER IF EXISTS update_balance_alerts_updated_at ON balance_alerts;
...
DROP TABLE IF EXISTS balance_alerts CASCADE;
DROP TABLE IF EXISTS whatsapp_config CASCADE;

-- ============================================
-- CRIAÇÃO LIMPA - Cria tudo novamente
-- ============================================
...
CREATE INDEX idx_balance_alerts_client ON balance_alerts(client_id);
CREATE INDEX idx_balance_alerts_active ON balance_alerts(is_active) WHERE is_active = true;
CREATE INDEX idx_balance_alerts_last_checked ON balance_alerts(last_checked_at);
...
CREATE INDEX idx_alert_recipients_client ON alert_recipients(client_id);
CREATE INDEX idx_alert_recipients_active ON alert_recipients(is_active) WHERE is_active = true;

-- ============================================
-- RLS POLICIES (usando memberships)
-- ============================================
...
CREATE VIEW active_balance_alerts AS
SELECT 
  ba.*,
