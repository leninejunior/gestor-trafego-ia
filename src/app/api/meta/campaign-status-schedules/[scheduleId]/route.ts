...
export async function PATCH(
  ...
  try {
    ...
    if (body.targetStatus !== undefined) {
      ...
      updates.target_status = targetStatus;
    }
    ...
    if (parsedDate !== null) {
      updates.scheduled_date = formatDatePartsToIso(parsedDate);
    }
    ...
    if (error || !data) {
      console.error('[Campaign Status Schedules][PATCH] Supabase error:', error);
    ...
    console.error('[Campaign Status Schedules][PATCH] Error:', error);
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('[Campaign Status Schedules][DELETE] Supabase error:', error);
    ...
    console.error('[Campaign Status Schedules][DELETE] Error:', error);
    return NextResponse.json({ error: 'Erro interno do servidor' }, { status: 500 });
  }
}
