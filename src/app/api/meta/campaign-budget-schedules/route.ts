...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[Campaign Budget Schedules][GET] Supabase error:', error);
      return NextResponse.json({ error: 'Falha ao carregar agendamentos' }, { status: 500 });
    }

    return NextResponse.json({ schedules: data ?? [] });
  } catch (error) {
    console.error('[Campaign Budget Schedules][GET] Error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const dailyBudget = parsePositiveBudget(body.dailyBudget);
    ...
    const dayOfWeek = new Date(`${scheduledDateIso}T00:00:00`).getDay();
    const serviceSupabase = createServiceClient();
    const { data, error } = await serviceSupabase
      .from('meta_campaign_budget_schedules')
      .insert({
    ...
    if (error) {
      console.error('[Campaign Budget Schedules][POST] Supabase error:', error);
    ...
    console.error('[Campaign Budget Schedules][POST] Error:', error);
    return NextResponse.json({ error: 'Erro interno do servidor' }, { status: 500 });
  }
}
