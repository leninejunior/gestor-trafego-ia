/**
 * Limpar e Popular banco com dados de teste para Sistema de Alertas de Saldo
 */
...
async function limparEPopular() {
  console.log('=== LIMPANDO E POPULANDO DADOS DE TESTE ===\n');

  try {
    // 1. Limpar dados existentes (em ordem reversa de dependências)
    console.log('1. Limpando dados existentes...');
    
    await supabase.from('alert_history').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabase.from('balance_alerts').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabase.from('ad_account_balances').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabase.from('client_meta_connections').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabase.from('clients').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabase.from('organizations').delete().eq('slug', 'agencia-teste');
    
    console.log('✓ Dados limpos');

    // 2. Criar organização de teste
    console.log('\n2. Criando organização de teste...');
    ...
    // 4. Criar conexões Meta Ads (múltiplas contas por cliente)
    console.log('\n4. Criando conexões Meta Ads...');
    const conexoes = [];
    ...
    // 5. Criar saldos das contas
    console.log('\n5. Criando saldos das contas...');
    const saldos = [];
    ...
    saldosCriados.forEach((saldo, index) => {
      alertas.push({
        ...
        alert_type: saldo.status === 'critical' ? 'no_balance' : 'low_balance',
        is_active: true
      });
    });
    ...
    // 7. Criar histórico de alertas
    console.log('\n7. Criando histórico de alertas...');
    const historico = [];

    alertasCriados.slice(0, 3).forEach((alerta, index) => {
      // Alerta enviado com sucesso por email
      historico.push({
        alert_id: alerta.id,
        sent_via: 'email',
        recipient: 'admin@teste.com',
        status: 'sent',
        message: `Alerta de saldo baixo enviado`,
        balance_at_send: 45.50 + (index * 10),
        threshold_at_send: alerta.threshold_amount,
        sent_at: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString() // 2 horas atrás
      });

      // Alerta via WhatsApp (para alguns)
      if (index % 2 === 0) {
        historico.push({
          alert_id: alerta.id,
          sent_via: 'whatsapp',
          ...
          status: 'sent',
          message: `Alerta de saldo baixo enviado via WhatsApp`,
          balance_at_send: 45.50 + (index * 10),
          threshold_at_send: alerta.threshold_amount,
